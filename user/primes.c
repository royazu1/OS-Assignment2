...
Generate: // we come back to this label when the user prompts us to generate again
    ...
    { // printer child proc
        ...
        while (num_printed < MAX_PRINT)
        {
            ...
            { // print channel was destroyed.
                // printf("Channel is unused - can't print! , exiting...\n");
                exit(1);
            }
            else
            { // taken prime from channel successfully
              // printf("The currently printed prime number is: %d\n" , prime_to_print);
            }
            num_printed++;
        }
        if (channel_destroy(print_cd) < 0)
        { // printed 100 primes already, try to destroy print channel
          // printf("Printing proc couldn't destroy print channel..\n");
        }
        else
        { // succesfully destroyed print chan
            printf("Printer proc with pid=%d destroyed print channel successfully\n", getpid());
        }
        exit(0);
    }
    else
    { // number genereator proc

        while (num_checkers > 0)
        { // generator proc creates num_checkers procs, then sends numbers to prime channel until chan is deleted
            if (fork() == 0)
            { // child checker - tries to take from prime channel and put in print channel, indefinitely.
                int checker_pid = getpid();
                while (1)
                { // take and put until printing proc destroys the channel
                    int curr_prime = -1;
                    if (channel_take(primes_cd, &curr_prime) == 0)
                    {
                        if (isPrime(curr_prime))
                        { // only put prime numbers for print channel
                            if (channel_put(print_cd, curr_prime) == 0)
                            { // put into PRINT chan was succseful
                                printf("Checker proc with pid=%d has put the prime number %d successfully\n", checker_pid, curr_prime);
                            }
                            else
                            { // print channel has been destroyed - race to destroy the prime channel
                                if (channel_destroy(primes_cd) < 0)
                                {
                                    // printf("Printing proc couldn't destroy print channel..\n");
                                }
                                else
                                { // prime chan destroyed succesfully
                    ...
                    { // take from prime channel failed, exit!
                        exit(0);
                    }
                }
            }
        ...
        while (1)
        {
            if (channel_put(primes_cd, num) < 0)
            { // put numbers to the prime channel indefinitely..
                char prompt;
                printf("Prime proc-Prime channel was destroyed, exiting..\n");
                printf("Do you want to generate again? Y/N\n");
                while (wait(0) != -1)
                    ;                // wait for all children procs to exit
                read(0, &prompt, 1); // here we finally know the stdin buffer is empty.. great success
                if (prompt == 'Y')
                {
                    while (read(0, &prompt, 1))
                    { // clean buffer so the read() call will block!
                        if (prompt == '\n')
                            break;
                        printf("%c", prompt);
                    }
